...
$string['wizard_hint_cloud_body']       = 'Ingrese la URL donde está desplegado saipa-engine (con Cloud API configurado) y el token <code>ENGINE_SECRET</code>. El motor utilizará su clave de API en la nube internamente.';
...
$string['wizard_hint_local_body']       = 'El puerto por defecto de saipa-engine es <code>8052</code>. Si lo está ejecutando con Docker en el mismo equipo, use <code>http://localhost:8052</code>. El token es opcional, salvo que haya configurado <code>ENGINE_SECRET</code> en <code>.env</code>.';
...
$string['wizard_js_troubleshoot_li3']   = 'Si usa un token, ¿coincide con <code>ENGINE_SECRET</code> en <code>.env</code>?';
...
$string['wizard_token_help']            = 'Valor de <code>ENGINE_SECRET</code> en el archivo <code>.env</code> del motor. Déjelo en blanco si no configuró un secreto.';
