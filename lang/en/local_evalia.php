...
$string['wizard_hint_cloud_body']       = 'Enter the URL where saipa-engine is deployed (with Cloud API configured), and the <code>ENGINE_SECRET</code> token. The engine will use your cloud API key internally.';
...
$string['wizard_hint_local_body']       = 'The default port for saipa-engine is <code>8052</code>. If you are running it via Docker on the same host, use <code>http://localhost:8052</code>. Token is optional unless you configured <code>ENGINE_SECRET</code> in <code>.env</code>.';
...
$string['wizard_js_troubleshoot_li3']   = 'If using a token, does it match <code>ENGINE_SECRET</code> in <code>.env</code>?';
...
$string['wizard_token_help']            = 'Value of <code>ENGINE_SECRET</code> in the engine\'s <code>.env</code> file. Leave blank if you did not configure a secret.';
