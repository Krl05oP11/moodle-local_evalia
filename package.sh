#!/usr/bin/env bash
# package.sh — Build a clean release ZIP for local_evalia.
#
# Usage:
#   ./package.sh          # reads version from version.php
#   ./package.sh 0.4.8    # override version label
#
# Output: ../local_evalia-<version>.zip  (one level up from the plugin dir)

set -euo pipefail

PLUGIN_DIR="$(cd "$(dirname "$0")" && pwd)"
PLUGIN_NAME="local_evalia"
