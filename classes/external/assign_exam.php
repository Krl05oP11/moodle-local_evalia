    ...
    public static function execute(int $examid): array {
        ...
        foreach ($studentids as $userid) {
            ...
            if (isset($sample['error'])) {
                // Transport / engine failure — every remaining student would fail
                // the same way. Stop instead of hammering a dead engine N times,
                // and report it as an engine problem (not "already had an exam").
                debugging('assign_exam: engine error — ' . $sample['error'], DEBUG_DEVELOPER);
                $enginedown = true;
                break;
            }

            if (empty($sample['selected_ids'])) {
                // The sampler ran but returned nothing for this student (e.g. the
                // approved bank cannot satisfy the requested difficulty mix).
                debugging('assign_exam: empty sample for userid=' . $userid, DEBUG_DEVELOPER);
                $failed++;
                continue;
            }
        ...
        if ($enginedown) {
            $message = get_string('error_engine_unreachable', 'local_evalia');
            if ($assigned > 0) {
                // Some students already got exams (and the template was activated)
                // before the engine went away — say so, don't just report failure.
                $message .= ' ' . get_string('exam_assign_partial', 'local_evalia', $assigned);
            }
            return [
                'success'  => false,
                'assigned' => $assigned,
        ...
        if ($failed > 0) {
            $msg .= ' Sin asignar (banco insuficiente): ' . $failed . '.';
        }
