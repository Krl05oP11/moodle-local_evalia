...
/**
 * Curl security helper that additionally trusts the configured EVAL-IA engine host.
 *
 ...
 * host: the EVAL-IA engine the site administrator configured in this
 * plugin's own settings (a page gated by moodle/site:config). Everything
 * else — including any other private/internal address — is still handled
 * by the stock curlsecurityblockedhosts list, completely unmodified.
 *
 * Duplicated from local_saipa\engine_security_helper on purpose: the two
 * plugins are sold and installed independently (no cross-plugin dependency
 * beyond block_saipa -> local_saipa), so local_evalia must not reach into
 * local_saipa's namespace for this.
 *
